...
existing_item=$(bw list items --search "Morph API Key" 2>/dev/null | jq -r '.[0].id // empty')

if [ -n "$existing_item" ]; then
    echo "⚠️  Morph API Key already exists in Bitwarden"
    ...
    echo -n "Enter your new Morph API key: "
    ...
    if bw get item "$existing_item" | jq \
      ...
      | bw encode | bw edit item "$existing_item"; then
        echo "✅ Morph API key updated in Bitwarden!"
    else
        echo "❌ Failed to update API key in Bitwarden"
        exit 1
    fi
else
    echo "Creating Morph API key in Bitwarden..."
    echo -n "Enter your Morph API key: "
    read -s api_key < /dev/tty
    echo ""
    ...
    if bw get template item | jq \
      --arg name "Morph API Key" \
      --arg username "morph" \
      --arg password "$api_key" \
      --arg notes "API key for Morph fast apply services used in avante.nvim" \
      '.type = 1 | .name = $name | .login.username = $username | .login.password = $password | .notes = $notes' \
      | bw encode | bw create item; then
        echo "✅ Morph API key stored in Bitwarden!"
    else
