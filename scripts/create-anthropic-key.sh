...
existing_item=$(bw list items --search "Anthropic API Key" 2>/dev/null | jq -r '.[0].id // empty')

if [ -n "$existing_item" ]; then
    echo "⚠️  Anthropic API Key already exists in Bitwarden"
    ...
    echo -n "Enter your new Anthropic API key: "
    ...
    if bw get item "$existing_item" | jq \
      ...
      | bw encode | bw edit item "$existing_item"; then
        echo "✅ Anthropic API key updated in Bitwarden!"
    else
        echo "❌ Failed to update API key in Bitwarden"
        exit 1
    fi
else
    echo "Creating Anthropic API key in Bitwarden..."
    ...
    if bw get template item | jq \
      --arg name "Anthropic API Key" \
      --arg username "anthropic" \
      --arg password "$api_key" \
      --arg notes "API key for Claude/Anthropic services used in avante.nvim" \
      '.type = 1 | .name = $name | .login.username = $username | .login.password = $password | .notes = $notes' \
      | bw encode | bw create item; then
        echo "✅ Anthropic API key stored in Bitwarden!"
    else
